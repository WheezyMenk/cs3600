...
def consistent(assignment, csp, var, value):
    ...
    for c in csp.binaryConstraints:
        if c.affects(var) :
            if assignment.assignedValues[c.otherVariable(var)] == value :
                return False
    return True
...
def minimumRemainingValuesHeuristic(assignment, csp):
    ...
    """Question 2"""
    """YOUR CODE HERE"""
    helper = [(key, len(domains[key])) for key in domains.keys() if assignment.assignedValues[key] is None]
    helper.sort(key=lambda x: x[1])
    mrvs = [key[0] for key in helper if key[1] == helper[0][1]]
    if len(mrvs) == 0 :
        return None
    if len(mrvs) == 1 or not csp.binaryConstraints:
        return mrvs[0]
    degree = 0
    for var in mrvs:
        curDegree = 0
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    """Question 3"""
    """YOUR CODE HERE"""
    affectedVars = [c.otherVariable(var) for c in csp.binaryConstraints if c.affects(var)]
    valuesAndConstraints = []
    for value in values :
        constraintCount = 0
        for affectedVar in affectedVars:
            if value in assignment.varDomains[affectedVar]:
                constraintCount += 1
        valuesAndConstraints.append((value, constraintCount))
    valuesAndConstraints.sort(key = lambda x : x[1])
    return [pair[0] for pair in valuesAndConstraints]
...
def forwardChecking(assignment, csp, var, value):
    ...
    """Question 4"""
    """YOUR CODE HERE"""
    affectedVars = [c.otherVariable(var) for c in csp.binaryConstraints if c.affects(var) and assignment.assignedValues[c.otherVariable(var)] is None]
    removedVars = []
    isReversed = False
    for affectedVar in affectedVars:
        if value in domains[affectedVar] :
            if len(domains[affectedVar]) > 1 :
                domains[affectedVar].remove(value)
                removedVars.append(affectedVar)
                inferences.add((affectedVar, value))
            else :
                """Reversing the inference"""
                for removedVar in removedVars :
                    domains[removedVar].add(value)
                isReversed = True
                break
    return inferences if not isReversed else None
    ...
    for value in orderValuesMethod(assignment, csp, var) :
        if consistent(assignment, csp, var, value) :
            ...
            if inferences is not None:
                ...
                if result is not None :
                    return result
        """Reversing the inferences"""
        if inferences:
            for (variable, varVal) in inferences:
                assignment.varDomains[variable].add(varVal)
        assignment.assignedValues[var] = None
    return None
...
def revise(assignment, csp, var1, var2, constraint):
    ...
    for val2 in assignment.varDomains[var2]:
        ...
        if not satisfied:
            ...
            removedCount += 1
            if removedCount >= len(assignment.varDomains[var2]):
                return None
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    q = deque()
    for c in csp.binaryConstraints:
        if c.affects(var) and assignment.assignedValues[c.otherVariable(var)] is None:
            q.append((var, c.otherVariable(var), c))
    while q:
        targetVar, affectedVar, c = q.pop()
        inferencesNew = revise(assignment, csp, targetVar, affectedVar, c)
        if inferencesNew is None:
            """Reversing inferences"""
            for variable, varValue in inferences:
                assignment.varDomains[variable].add(varValue)
            return None
        elif inferencesNew :
            for c in csp.binaryConstraints:
                if c.affects(affectedVar) and assignment.assignedValues[c.otherVariable(affectedVar)] is None:
                    q.append((affectedVar, c.otherVariable(affectedVar), c))
...
def AC3(assignment, csp):
    ...
    for key in csp.varDomains.keys() :
        for c in csp.binaryConstraints:
            if (c.affects(key)) :
                q.append((key, c.otherVariable(key), c))
    while len(q) > 0 :
        var, affectedVar, c = q.pop()
        inferencesNew = revise(assignment, csp, var, affectedVar, c)
        if inferencesNew is None:
            """Reversing inferences"""
            for variable, varValue in inferences:
                assignment.varDomains[variable].add(varValue)
