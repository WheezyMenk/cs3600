    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            for p in self.legalPositions:
                allPossible[p] = self.beliefs[p] * emissionModel[util.manhattanDistance(pacmanPosition, p)]
        else:
            '''Ghost captured'''
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                curBeliefs[newPos] += self.beliefs[p] * prob
        self.beliefs = curBeliefs
...
class ParticleFilter(InferenceModule):
        ...
        while num < self.numParticles:
            for p in self.legalPositions:
                if num < self.numParticles:
                    self.particlePositions.append(p)
                    num += 1
                else:
                    break
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None: 
            '''Ghost Captured, fill list with jail positions'''
            self.particlePositions = [self.getJailPosition()] * self.numParticles
        else:
            curBelief = util.Counter()
            distr = self.getBeliefDistribution()
            for pos in self.legalPositions:
                curBelief[pos] += distr[pos] * emissionModel[util.manhattanDistance(pacmanPosition, pos)]
            if all(x == 0 for x in curBelief.values()):
                '''all receive 0 weight'''
                self.initializeUniformly(gameState)
            else:
                '''resample particles uniformly'''
                self.particlePositions = [util.sample(curBelief) for i in range(self.numParticles)]
    def elapseTime(self, gameState):
        """
        ...
        for pos in self.particlePositions:
            ...
            newParticlePositions.append(util.sample(newPosDist))
        self.particlePositions = newParticlePositions
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    ...
                    prob *= emissionModels[i][util.manhattanDistance(pacmanPosition, particle[i])]
            curBelief[particle] += prob
        ...
        if all(x == 0 for x in curBelief.values()):
            ...
            for pos in self.particles:
                for i in range(self.numGhosts):
                    if noisyDistances[i] is None:
                        particle = self.getParticleWithGhostInJail(pos, i)
        else:
            self.particles = [util.sample(curBelief) for i in range(self.numParticles)]
