...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        positions = [distr.argMax() for distr in livingGhostPositionDistributions]
        action = None
        actProb = 0
        for i, pos in enumerate(positions):
            prob = livingGhostPositionDistributions[i][pos]
            if prob > actProb:
                action = pos
                actProb = prob
        ...
        for pacAction in legal:
            ...
            disAndActions.append((pacAction, self.distancer.getDistance(successorPosition, action)))
        '''Sort by distance'''
        disAndActions.sort(key=lambda x: x[1])
        return disAndActions[0][0]
