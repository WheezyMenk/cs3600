...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    curTree = Tree(self.root.children[classificationData[self.root.attr]])
...
def getPertinentExamples(examples,attrName,attrValue):
    ...
    newExamples = []
    #YOUR CODE HERE
    newExamples = [example for example in examples if example[attrName] == attrValue]
    return newExamples
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        curExamples = getPertinentExamples(examples, attrName, attrValue)
        attributeCounts[attrValue] = getClassCounts(curExamples, className)
...
def remainder(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        curExamples = getPertinentExamples(examples, attrName, attrValue)
        curClassCounts = getClassCounts(curExamples, className)
        ret += float(sum(curClassCounts.values())) / total * setEntropy(curClassCounts.values())
    return ret
...
def giniGain(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        curExamples = getPertinentExamples(examples, attrName, attrValue)
        classCounts = getClassCounts(curExamples, className)
        ret += float(len(curExamples)) / len(examples) * giniIndex(classCounts.values())
    if ret == 0:
        import sys
        return sys.maxint 
    else:
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    for attr in remainingAttributes:
        if gainFunc(examples, attr, attributeValues[attr], className) > gainFunc(examples, argmax, attributeValues[argmax], className):
            argmax = attr
    root = Node(argmax)
    curAttributes = list(remainingAttributes)
    curAttributes.remove(argmax)
    for attrValue in attributeValues[argmax]:
        curExamples = getPertinentExamples(examples, argmax, attrValue)
        root.children[attrValue] = makeSubtrees(curAttributes, curExamples, attributeValues, className, getMostCommonClass(examples, className), setScoreFunc, gainFunc)
    return root
